# 📊 طراحی کامل DDL - 38 جدول

**نسخه:** 2.0 | **تاریخ:** 1403/04/27

## جداول Layer 1: Authentication
...
## جداول Layer 3-4: Accounts و Trading
...
## جداول Layer 6-8: Market Data, Risk, Notifications
...
### trading_journal (یادداشت‌های تجاری)
```sql
...
### strategy_configurations (پارامترهای استراتژی)
```sql
...
**Priority 1 (الویت بالا):**
- users (email, username, status)
- licenses (user_id, status, expiration)
- trading_accounts (user_id, balance)
- trades (account, pair, status, time)
- signals (pair, source, status, time)

**JSONB Indexes (GIN):**
- signals.affected_pairs
- strategies.parameters
- optimization_results.parameter_set
- ai_decisions.factors

---

## 🔐 امنیت

✅ Encryption: password_hash (bcrypt), API keys (AES-256)
✅ Audit Trail: تمام تغییرات logged
